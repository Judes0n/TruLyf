...
export class PenaltyComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.penalties = [];
    let allpenalties = (await this.clientservice.GetPenalties(this.clientpolicyId).toPromise());
    allpenalties.forEach(pen => {
      if(pen.status == PenaltyStatusEnum.Pending)
        this.penalties.push(pen);
    });
    var amount = this.penalties[0].penalty;
  ...
  payment() {
    ...
    this.clientservice.MakePayment(payreq,1).subscribe((res: Payments) => {
